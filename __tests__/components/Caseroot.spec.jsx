...
import {
  Caseroot, mapDispatchToProps, mapStateToProps, loadModuleData,
} from '../../src/components/Caseroot';
...
describe('Caseroot should render as expected', () => {
  it('module should render correct JSX', () => {
    ...
    const renderedModule = shallow(<Caseroot {...props} />);
  ...
  describe('mapDispatchToProps', () => {
    it('should update the browser locale and then reload the language pack', async () => {
      ...
      expect(mockDispatch).toHaveBeenNthCalledWith(2, 'I am loading the language pack for caseroot and my fallback locale is en-US');
    });
  });
  ...
  describe('loadModuleData', () => {
    ...
    it('should load language pack for caseroot module ', async () => {
      const langPackAsyncState = await loadModuleData({ store: fakeStore });
      expect(langPackAsyncState).toBe('I am loading the language pack for caseroot and my fallback locale is en-US');
